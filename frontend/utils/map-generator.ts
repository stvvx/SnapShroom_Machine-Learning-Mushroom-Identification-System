...
/**
 * Generate interactive HTML map for web platform using Google Maps
 */
  ...
  <script>
    ...
    function initMap() {
      ...
      locations.forEach(loc => {
        const markerSize = loc.prevalence === 'high' ? 16 : 
                          loc.prevalence === 'medium' ? 12 : 9;
        ...
        marker.addListener('click', () => {
          infoWindow.setContent(\`
            <div style="min-width:200px; font-family: -apple-system, sans-serif;">
              <h3 style="margin:0 0 8px 0; color:#2E7D32;">\${loc.name}</h3>
              <p style="margin:4px 0;"><strong>Prevalence:</strong> \${loc.prevalence.toUpperCase()}</p>
              <p style="margin:4px 0;">\${loc.cultivated ? '🌱 Cultivated' : '🌲 Wild'}</p>
              \${loc.notes ? '<p style="margin:8px 0 0 0; font-size:12px; color:#666;">📝 ' + loc.notes + '</p>' : ''}
            </div>
          \`);
...
/**
 * Generate map for native platforms using Google Maps
 */
